...
export default function LeaderNav({ user }: { user: SessionUser }) {
  const pathname = usePathname();
  const router = useRouter();

  const links = [
    { href: "/team", label: "Team Overview" },
    { href: "/team/icp", label: "ICP Fit" },
    { href: "/team/calls", label: "All Calls" },
    { href: "/team/request-investigation", label: "Request Investigation" },
  ];
